...
from arena import get_physics, start_arena, max_velocity, max_velocity_change, max_angle_change, too_close, image_size
...
def get_reward(agent, dist, closer, vel, step, collision, done, printing = False):
    ...
    r = r_dist + r_closer + r_vel + r_step + r_col
    if(done):
    ...
    if(printing):
        print("\n{} reward {}:\n\t{} from dist,\n\t{} from dist closer,\n\t{} from vel,\n\t{} from step,\n\t{} from collision.".format(
            agent, round(r,3), round(r_dist,3), round(r_closer,3), round(r_vel,3), round(r_step, 3), round(r_col, 3)))
    return(r)
...
class PredPreyEnv():   
    def __init__(
        ...
        self.test = test
        self.GUI = GUI
        if(self.GUI):   self.physicsClient = get_physics(self.GUI, self.arena)
    ...
    def reset(self):
        self.close()
        self.resets += 1
        self.pred, self.prey, self.pos_pred, self.pos_prey, self.yaw_pred, self.yaw_prey, self.vel_pred, self.vel_prey, wall_ids = \
            start_arena(self.test, self.physicsClient, self.arena, self.already_constructed)
    ...
    def change_angle_vel(self, agent, yaw, vel):
        if(agent == self.pred): old_yaw, old_vel, pos = self.yaw_pred, self.vel_pred, self.pos_pred
        if(agent == self.prey): old_yaw, old_vel, pos = self.yaw_prey, self.vel_prey, self.pos_prey
        ...
        new_vel = old_vel + vel
        new_vel = sorted((0, max_velocity, new_vel))[1]
        x = cos(new_yaw)*new_vel
        y = sin(new_yaw)*new_vel
        p.resetBaseVelocity(agent, (x,y,0), (0,0,0), physicsClientId = self.physicsClient)
        #print("Change velocity:", round(vel))
        #print("Old:", round(old_vel))
        #print("New:", round(new_vel))
        
        if(agent == self.pred): self.yaw_pred = new_yaw; self.vel_pred = new_vel
        if(agent == self.prey): self.yaw_prey = new_yaw; self.vel_prey = new_vel
        
    def get_vel(self, agent):
    ...
    def step(self, ang_vel_1 = None, ang_vel_2 = None):
        ...
        if(self.pred_condition == "random"): 
            ...
            vel_1   = random.uniform(-max_velocity_change,max_velocity_change)
        # Otherwise, make sure the given movement is within allowance. 
        else:
            if(self.normalize):
                angle_1 = -max_angle_change + (ang_vel_1[0] + 1.0) * 0.5 * (max_angle_change - -max_angle_change)
                vel_1   = -max_velocity_change + (ang_vel_1[1] + 1.0) * 0.5 * (max_velocity_change - -max_velocity_change)
            else:   
                angle_1 = ang_vel_1[0] 
                vel_1   = ang_vel_1[1] 
            angle_1 = sorted((-max_angle_change, max_angle_change, angle_1))[1]
            vel_1   = sorted((-max_velocity_change, max_velocity_change, vel_1))[1]
        if(self.prey_condition == "random"): 
            angle_2 = random.uniform(-max_angle_change,max_angle_change)
            vel_2   = random.uniform(-max_velocity_change,max_velocity_change)
        else:
            if(self.normalize):
                angle_2 = -max_angle_change + (ang_vel_2[0] + 1.0) * 0.5 * (max_angle_change - -max_angle_change)
                vel_2   = -max_velocity_change + (ang_vel_2[1] + 1.0) * 0.5 * (max_velocity_change - -max_velocity_change)
            else:   
                angle_2 = ang_vel_2[0] 
                vel_2   = ang_vel_2[1] 
            angle_2 = sorted((-max_angle_change, max_angle_change, angle_2))[1]
            vel_2   = sorted((-max_velocity_change, max_velocity_change, vel_2))[1]
        self.steps += 1
        dist_before = self.agent_dist()
        self.change_angle_vel(self.pred, angle_1, vel_1)
        self.change_angle_vel(self.prey, angle_2, vel_2)
        if(self.pred_condition == "still" or self.pred_energy <= 0): p.resetBaseVelocity(self.pred, (0,0,0), (0,0,0), physicsClientId = self.physicsClient); self.vel_pred = 0
        if(self.prey_condition == "still" or self.prey_energy <= 0): p.resetBaseVelocity(self.prey, (0,0,0), (0,0,0), physicsClientId = self.physicsClient); self.vel_prey = 0
        ...
        self.vel_pred = self.get_vel(self.pred)
        self.vel_prey = self.get_vel(self.prey)
        dist_after = self.agent_dist()
        dist_closer = dist_before - dist_after
        pred_collision, prey_collision = self.collisions()
        self.pred_energy -= self.vel_pred+5
        self.prey_energy -= self.vel_prey+5
        done = True if dist_after <= too_close or self.pred_energy <= 0 else False
        reward = (
            get_reward("pred", dist_after, dist_closer, self.vel_pred, self.steps, pred_collision, done), 
            get_reward("prey", dist_after, dist_closer, self.vel_prey, self.steps, prey_collision, done))
        #if(done): print("\n\n\tEnded!\n\n")
...
def run_with_GUI(test = False, pred = None, prey = None, GUI = True, episodes = 100, pred_condition = False, prey_condition = False, arena = "arena.png", render = False):
    env = PredPreyEnv(test = test, GUI = GUI, pred_condition = pred_condition, prey_condition = prey_condition, arena = arena)
    win_list = []
    ...
    for i in tqdm(range(episodes), desc = "Testing", position=0, leave=True):
        ...
        while(done == False):
            steps += 1
            if(pred != None):
                ang_vel_1, pred_actor_hc = pred.get_action(obs[0], env.vel_pred, env.pred_energy, ang_vel_1, pred_actor_hc)
            else:
                ang_vel_1 = (None, None)
            if(prey != None):
                ang_vel_2, prey_actor_hc = prey.get_action(obs[1], env.vel_prey, env.prey_energy, ang_vel_2, prey_actor_hc)
            else:
                ang_vel_2 = (None, None)
            obs, _, done, dist_after = env.step(ang_vel_1, ang_vel_2)
...
if __name__ == "__main__":
    env = PredPreyEnv(arena = "bigger_arena.png")
    env.reset()   
    env.render() 
    print("\nObservation space:")
    ...
    print()
    env.close(forever = True)
    run_with_GUI(episodes = 100, arena = "arena.png", 
